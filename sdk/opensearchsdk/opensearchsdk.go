...
func (this *OpenSearchSdk) encodeUrl(input string) (string, error) {
	output, err := encoding.EncodeUrl(input)
	if err != nil {
		return "", err
	}
...
func (this *OpenSearchSdk) getSignature(method string, query map[string]string) (map[string]string, error) {
	...
	queryKeyInterface, _ := language.ArrayKeyAndValue(newQuery)
	queryKey := language.ArraySort(queryKeyInterface).([]string)
...
func (this *OpenSearchSdk) api(method string, url string, query map[string]string, bodyResult interface{}) error {
	...
	newQuery["SignatureNonce"] = crypto.CryptoRand(17)
	...
	if method == "GET" {
		err = util.DefaultAjaxPool.Get(&ajax)
	} else {
		err = util.DefaultAjaxPool.Post(&ajax)
	}
	if err != nil {
		return err
	}
...
func (this *OpenSearchSdk) combineQuery(request interface{}) map[string]string {
	data := language.ArrayToMap(request, "query")
