...
func (this *YouzanSdk) getSign(data map[string]interface{}) (string, error) {
	dataKeysInterface, _ := language.ArrayKeyAndValue(data)
	dataKeys := dataKeysInterface.([]string)
	dataKeys = language.ArraySort(dataKeys).([]string)
	...
	return crypto.GetMd5String(result), nil
}
...
func (this *YouzanSdk) api(method string, data interface{}, responseData interface{}) error {
	//调整输入参数
	queryParamsInterface := language.ArrayToMap(data, "url")
	...
	//请求
	var dataByte []byte
	queryBytes, err := encoding.EncodeUrlQuery(queryParams)
	if err != nil {
		return err
	}
	...
	err = encoding.DecodeJson(dataByte, &responseMap)
	if err != nil {
		return err
	}
	if errorData, isErr := responseMap["error_response"]; isErr {
		var youzanErr YouzanSdkError
		err := language.MapToArray(errorData, &youzanErr, "json")
		if err != nil {
			return err
		} else {
			return &youzanErr
	...
	err = language.MapToArray(responseMap["response"], responseData, "json")
	if err != nil {
		return err
	}
	return nil
...
func (this *YouzanSdk) GetOauthUrl(redirectUrl string, scope string, state string) (string, error) {
	...
	inputDataStr, err := encoding.EncodeUrlQuery(inputData)
	if err != nil {
		return "", err
	}
...
func (this *YouzanSdk) GetOauthUserInfo(requestUrl *url.URL) (YouzanSdkOauthUserInfo, error) {
	...
	err := encoding.DecodeUrlQuery([]byte(urlQuery), &inputRequest)
	if err != nil {
	...
	err = language.MapToArray(inputRequest, &result, "url")
	if err != nil {
		return YouzanSdkOauthUserInfo{}, err
	}
	return result, nil
